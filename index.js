	...
	try {
		...
		if (isGroupMsg && isxp && !gaming.isWin(user) && !isBlocked) {
			try {
				...
				if (functions[0].dragon.includes(user)) { gainedXP = parseInt(gainedXP + (usuarioLevel * 5), 10) } // Beneficio de guilda dragon, XP 5x mais
				if (functions[0].wolves.includes(user)) { gainedXP = parseInt(gainedXP + (usuarioLevel * 3), 10) } // Beneficio de guilda wolves, XP 3x mais
		   ...
		   case 'clima':
			...
			case 'profile':;case 'perfil':
				if (isGroupMsg) {
					...
					if (functions[0].wolves.includes(qmid)) { myGuild = '\n\n⚔️ *Guilda:* wolves' } else if (functions[0].dragon.includes(qmid)) { myGuild = '\n\n⚔️ *Guilda:* dragon' }
			...
			case 'steal':
				...
				if (functions[0].wolves.includes(theStealK)) { lvpc = parseInt(lvpc + (getUsrLevel / 3), 10) } // Beneficio da Guilda de Ladrões, Steal melhora a cada nível, sem limite
				if (functions[0].dragon.includes(theStealK)) { lvpc = parseInt(lvpc + (getUsrLevel / 5), 10) } // Beneficio da Guilda dragon, melhora o Steal mas com limitação
			...
			case 'guild':;case 'guilda':
				...
				if (args.length !== 0 && args[0].toLowerCase() == '-wolves' || args.length !== 0 && args[0].toLowerCase() == '-dragon') {
					if (functions[0].wolves.includes(user) && args[0].toLowerCase() == '-wolves' || functions[0].dragon.includes(user) && args[0].toLowerCase() == '-dragon') return await kill.reply(from, mess.onGuild(), id)
					if (functions[0].wolves.includes(user) || functions[0].dragon.includes(user)) { await kill.reply(from, mess.changeGuild(), id);functions[0].dragon.splice(user, 1);functions[0].wolves.splice(user, 1) }
					if (args[0].toLowerCase() == '-wolves') { functions[0].wolves.push(user) } else if (args[0].toLowerCase() == '-dragon') { functions[0].dragon.push(user) }
					await fs.writeFileSync('./lib/config/Gerais/functions.json', JSON.stringify(functions))
			...
			case 'wolves':;case 'black wolves':
				if (!isGroupMsg) return await kill.reply(from, mess.sogrupo(), id)
				let claladrao = '----- [ *GUILDA BLACK WOLVES* ] -----\n\n'
				try {
					for (let i = 0; i < functions[0].wolves.length; i++) {
						const identladrao = await kill.getContact(functions[0].wolves[i])
						var getLadrao = identladrao.pushname == null ? 'wa.me/' + functions[0].wolves[i].replace('@c.us', '') : identladrao.pushname + ` - [wa.me/${functions[0].wolves[i].replace('@c.us', '')}]`
			...
			case 'dragon':;case 'dragon hunters':
				if (!isGroupMsg) return await kill.reply(from, mess.sogrupo(), id)
				let clacompnos = '----- [ *GUILDA DRAGON HUNTERS* ] -----\n\n'
				try {
					for (let i = 0; i < functions[0].dragon.length; i++) {
						const aGoodMan = await kill.getContact(functions[0].dragon[i])
						var butNotReal = aGoodMan.pushname == null ? 'wa.me/' + functions[0].dragon[i].replace('@c.us', '') : aGoodMan.pushname + ` - [wa.me/${functions[0].dragon[i].replace('@c.us', '')}]`
						clacompnos += `${i + 1} → *${butNotReal}*\n\n`
					}
					await kill.sendText(from, clacompnos)
				} catch (error) { 
					await kill.reply(from, mess.fail(), id)
					console.log(color('[DRAGON HUNTERS]', 'crimson'), color(`→ Obtive erros no comando ${prefix}${command} → ${error.message} - Você pode ignorar.`, 'gold'))
				}
				break
